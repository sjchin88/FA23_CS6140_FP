...
# %%[markdown]
# Function to show the correlation
...
def print_correlation(data):
    ...
    def print_corr_for(target):
        ...
        print(f'Correlation for {target}, sorted by absolute value')
        print("Sorted Pearson Correlation:\n",
              sorted_pearson.to_string(index=False))
        print("Sorted Spearman Correlation:\n",
              sorted_spearman.to_string(index=False))

    print_corr_for('target_wap')
    print_corr_for('target_index')
...
model_save_path = "initial_run_featuresCombo"
# %%
cross_validation(
    model_wap,
    model_idx,
